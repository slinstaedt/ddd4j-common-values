...
public class FileLog implements Log<Input>, ColdSource<Committed<Input>> {

	private final Scheduler scheduler;
	private FileChannel channel;
	private final Committer<Input> committer;

	public FileLog(Scheduler scheduler, FileChannel channel) throws IOException {
		this.scheduler = Require.nonNull(scheduler);
		this.committer = scheduler.createActorDecorator(Type.of(Committer.class), new Committer<Input>() {

			private final long currentOffset = channel.size();

			@Override
			public Outcome<CommitResult<Input>> tryCommit(Uncommitted<Input> attempt) {
	...
	@Override
	public ColdSource.Connection<Committed<Input>> open(Revision startAt, boolean completeOnEnd) throws Exception {
		return new ColdSource.Connection<Committed<Input>>() {

			private final long offset = startAt.offset();

			@Override
			public Seq<Committed<Input>> request(int n) throws Exception {
	...
	@Override
	public Result<Committed<Input>> publisher(Revision startAt, boolean completeOnEnd) {
